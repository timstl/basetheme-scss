...
function basetheme_enqueue() {
	...
	wp_enqueue_style( 'base-style', get_template_directory_uri() . '/dist/style.css', array(), '1.0' );

	wp_deregister_script( 'jquery' );
	wp_register_script( 'jquery', get_template_directory_uri() . '/js/head/vendor/jquery.min.js', array(), '3.3.1', false );
	wp_enqueue_script( 'jquery' );

	// gravity forms throws errors without jquery-migrate
	wp_enqueue_script( 'jquery-migrate' );

	wp_enqueue_script( 'head', get_template_directory_uri() . '/dist/head.min.js', array(), '1.0', false );

	if ( is_singular() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );}

	wp_enqueue_script( 'scripts', get_template_directory_uri() . '/dist/scripts.min.js', array(), '1.0', true );
}
